...
func Register(name string, f Factory) {
	...
	if f == nil {
		panic(errors.New("form captcha: Register form captcha factory is nil"))
	}
	if _, dup := factories[name]; dup {
		panic(errors.New("form captcha: Register called twice for factory " + name))
	}
	factories[name] = f
}
...
func NewDriver(name string, loader func(v interface{}) error) (Driver, error) {
	...
	factorysMu.RUnlock()
	if !ok {
		return nil, fmt.Errorf("form captcha: unknown driver %q (forgotten import?)", name)
	}
